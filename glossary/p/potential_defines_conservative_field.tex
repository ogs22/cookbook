...
Proof: Let f(x) be a field with potential $ \phi ( \mathrm{x} ) , $ and let
C be a curve. Then:
...
\[ = \phi ( \mathrm{x} _0 ) - \phi ( \mathrm{x} _1 ) \] 
So the value of the integral depends only on the endpoints of C, and not
on the shape of C. In particular, if C is a closed curve then this integral 
is equal to zero - so f(x) is conservative.